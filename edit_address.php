...
// fetch address data
$stmt = $conn->prepare("SELECT * FROM user_address WHERE Address_ID = ? AND User_ID=?");
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
   ...
   if(empty($errors)) {    
        if($is_default) { 
            ...
            $reset->bind_param('i', $user_id, $address_id);
            $reset->execute();
        }
        $update= $conn->prepare('UPDATE user_address
        SET Full_Name=?, Phone=?, State=?, postcode=?, Address=?, Unit_No=?, Delivery_Notes=?, Label=?, Is_Default=?, 
        WHERE Address_ID=? AND User_ID=?');
        $update->bind_param('ssssssssi', $full_name, $phone, 
        $state, $postcode, $address, $unit_no, 
        $delivery_notes, $label, $is_default, $address_id, $user_id);
...
<body>
    ...
    <session class="register-page">
        <div class="register-container">
            <div class="form-header">
                <h2 class="register-title">Edit Product</h2>
